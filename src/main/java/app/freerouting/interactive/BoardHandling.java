...
public class BoardHandling extends BoardHandlingHeadless {
   ...
   */
  public BoardHandling(BoardPanel p_panel, Locale p_locale, boolean p_save_intermediate_stages,
  ...
  void set_layer(int p_layer_no) {
    ...
    if (this.panel != null && (!this.board_is_read_only) && (curr_layer.is_signal)) {
   ...
   * p_item_observers and
   * p_item_id_no_generator are used, in case the board is embedded into a host
   * system. Returns
   * false, if the dsn-file is corrupted.
   */
  public DsnFile.ReadResult loadFromSpecctraDsn(InputStream p_design, BoardObservers p_observers,
    ...
    if (read_result == DsnFile.ReadResult.OK) {
      FRAnalytics.fileLoaded("DSN", this.board.communication.specctra_parser_info.host_cad + ","
   ...
   * format. If
   * p_compat_mode is true, only standard specctra dsn scopes are written, so that
   * any host system
   ...
   * already in there,
   * otherwise adds it to the list. Changes to the selected items state, if
   ...
   * filter. If nothing
   * is found on the active layer and settings.select_on_all_layers is true, all
   * layers are
   * selected.
   */
